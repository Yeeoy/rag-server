...
def _always_true(_model: str) -> bool:
    return True

# Monkey patch：强制认为所有模型都支持 image
import langchain_openai.chat_models.base as base

base._is_vision_model = _always_true

# Initialize LLM for summarization
llm = ChatOpenAI(model=os.getenv("OPENAI_DEFAULT_MODEL"), temperature=0)

# Embeddings model will be initialized lazily when needed
_embeddings_model = None


def get_embeddings_model():
    """Lazy initialization of embeddings model to avoid loading on import"""
    global _embeddings_model
    if _embeddings_model is None:
        print("🔄 Initializing embeddings model (first use)...")
...
def process_document(document_id: str):
    ...
    try:
        ...
        update_status(document_id, "summarising", {"chunking": chunking_metrics})
        ...
        print(
            f"✅ Celery task completed for document: {document_id} with {len(stored_chunk_ids)} chunks"
        )
...
def download_and_partition(document_id: str, document: dict):
    ...
    update_status(
        document_id, "chunking", {"partitioning": {"elements_found": elements_summary}}
    )
    os.remove(temp_file)
...
def analyze_elements(elements):
    ...
    for element in elements:
        element_name = type(
            element
        ).__name__  # Get the class name like "Table" or "NarrativeText"
...
def store_chunks_with_embeddings(document_id: str, processed_chunks: list):
    ...
    # Initialize embeddings model only when needed
