...
async def get_project_files(project_id: str, clerk_id: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get project files: {str(e)}"
        )
...
async def get_upload_url(
    ...
    try:
        ...
        if not projects_result.data:
            raise HTTPException(
                status_code=400, detail=f"Project not found or access denied"
            )

        file_extension = (
            file_request.filename.split(".")[-1] if "." in file_request.filename else ""
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate the presigned url: {str(e)}"
        )


@router.post("/api/projects/{project_id}/files/confirm")
async def confirm_file_upload(
    project_id: str, confirm_request: dict, clerk_id: str = Depends(get_current_user)
):
    try:
        s3_key = confirm_request.get("s3_key")

        if not s3_key:
            raise HTTPException(status_code=400, detail=f"s3_key is required")
        ...
        if not result.data:
            raise HTTPException(
                status_code=404, detail=f"Document not found or access denied"
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to confirme to upload")
...
async def delete_file(
    ...
    try:
        ...
        if not file_result.data:
            raise HTTPException(
                status_code=404, detail=f"file not found or access denied"
            )
