...
def get_projects(clerk_id: str = Depends(get_current_user)):
    try:
        result = (
            supabase.table("projects").select("*").eq("clerk_id", clerk_id).execute()
        )

        return {"message": "Projects retrieved successfully", "data": result.data}
...
def create_project(project: ProjectCreate, clerk_id: str = Depends(get_current_user)):
    try:
        ...
        return {"message": "Project created successfully", "data": created_project}
